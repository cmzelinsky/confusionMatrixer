...
"T":[(u'PAST',),(u'RECENTPAST',), (u'Prefix-Historical',), (u'FUTURE',), (u'CURRENT',), (u'PRESENT',), (u'UNDEFINED',)],
...
for doc in docs:
    if not doc.endswith('.out.xml'):
        ...
        for entry in entries:
            ...
            for child in entry.firstChild.childNodes:
                if child.localName == 'binding':
                    ...
                    value = [child.getAttribute('code') for child in entry.firstChild.childNodes if (child.localName == 'code' and child.getAttribute('code') != '\\' and child.getAttribute('displayName') != "Lifelong")]
                    value.extend([child.getAttribute('displayName') for child in entry.firstChild.childNodes if (child.localName == 'code' and child.getAttribute('code') == '\\')])
                    if len(entries) == 0:
                        continue
                    else:
        ...
        for entry in gsDic[doc]:
            print "entry: ", entry
            if entry not in engDic[doc] and gsDic[doc][entry] != ():
                print "gsDic[doc][entry]: ", gsDic[doc][entry]
...
if truePosCount == 0:
    truePosCount = 0.000000001
if falseNegCount == 0:
    falseNegCount = 0.000000001
if falsePosCount == 0:
    falsePosCount = 0.000000001
...
#print "1-token rule makes recall closer to... ", str(float(truePosCount)/float(truePosCount+(falseNegCount-63)))
...
if modifierType == "D":
    ...
    engineName = "Temporality"
elif modifierType == "C":
    engineName = "Certainty"
elif modifierType == "S":
    engineName = "Subject"
...
title = TElement('title', text= engineName + " Stats Results", parent=head)
...
h1 = TElement('h1', text= engineName + " Stats Results:", parent=body)
...
spontTitle = TElement('h2', text="Non-mismatch False Positives (if any):", parent=tr)
for column in values:
    th = TElement('th')
...
with open(os.path.join(path, "confusionMatrix-" + engineName + ".html"), 'w') as outputFile:
...
for doc in diffsDic:
    print "Now comparing content nodes for " + doc
                ...
                if entry in diffsDic[doc]['FP'].keys():
                    ...
                    if entry not in gsDic[doc].keys():
                                    ...
                                    if k >= 0 and not k > len(wordDict):
                                        if k == i:
                                            ...
                                            for m in range(0, len(entry)):
                                                if 'entry_'+str(i+m) in wordDict:
                                                    concept.append(wordDict['entry_'+str(i+m)])
                                            fpConcept = "".join(concept) #added here just for new deid engine to pass -- what's going wrong with entry_102? :
                        ...
                        #single-code spontaneous multi-entry
                            ...
                            while (k < (i + 10)):
                                if k >= 0 and not k > len(wordDict):
                                    if k == i: #added here just for new deid engine to pass -- what's going wrong with entry_102?
                                        concept = []
                                        for m in range(0, len(entry)):
                                            if 'entry_'+str(i+m) in wordDict:
                                                concept.append(wordDict['entry_'+str(i+m)])
                                        fpConcept = "".join(concept)
                                        temp.append('<font style="background-color:red;color:white;"><strong>' + fpConcept + '</strong></font>')
                                        k += len(entry)
                                        continue
                                    elif 'entry_'+str(k) in wordDict:
                                        temp.append(wordDict['entry_' + str(k)])
                            ...
                            if k == i:
                                for m in range(0, len(entry)):
                                    ...
                                    #Non-overlaps Handling -- everything will just be gold
                                        ...
                                        #quick fix to get Temporality up:
                                        if 'entry_'+str(i+m) in wordDict:
...
print '\nDetails file(s) generated -- written to ' + path + """ as "confusionMatrix-""" + engineName + """".html"""
